...
func TestSpannerSequencerAssignEntries(t *testing.T) {
	...
	want := uint64(0)
	for chunks := range 10 {
		entries := []*shizzle.Entry{}
		for i := range 10 + chunks {
			entries = append(entries, shizzle.NewEntry(fmt.Appendf(nil, "item %d/%d", chunks, i)))
		}
	...
	for _, test := range []struct {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			for i := range test.initialEntries {
				entries = append(entries, shizzle.NewEntry(fmt.Appendf(nil, "initial item %d", i)))
			}
			if err := seq.assignEntries(ctx, entries); err != nil {
			...
			entries = []*shizzle.Entry{shizzle.NewEntry([]byte("additional"))}
...
func TestSpannerSequencerRoundTrip(t *testing.T) {
	...
	for chunks := range 10 {
		entries := []*shizzle.Entry{}
		for range 10 + chunks {
			e := shizzle.NewEntry(fmt.Appendf(nil, "item %d", seq))
...
func makeBundle(t *testing.T, idx uint64, size int) []byte {
	...
	for i := range size {
		e := shizzle.NewEntry(fmt.Appendf(nil, "%d:%d", idx, i))
